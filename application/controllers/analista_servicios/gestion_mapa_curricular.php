...
class gestion_mapa_curricular extends CI_Controller {
     ...
     * @return View
     */
    public function mc_pe($idinstitucion, $idpe) {
         ...
         */
        $data['breadcrumbs']['links'] = array(array('AIDE', '#'), array('Inicio', base_url() . 'app/inicio'), array('Consultar instituciones', base_url() . 'analista_servicios/gestion_instituciones'), array('Planes de estudio', base_url() .'analista_servicios/gestion_planes_estudios/institucion/'. $idinstitucion), 'Mapa curricular');
         ...
         */
        $data['app_fragment'] = $this->load->view('app/private/fragments/modules/Modulo2/analista_servicios/gestion_mapa_curricular_view', $data, TRUE);
    ...
    public function crear_mapa_curricular() {
        ...
        if ($this->form_validation->run()) {
            ...
            redirect(base_url() . "analista_servicios/gestion_mapa_curricular/mc_pe/" . $idinstitucion . '/' . $idpe, 'refresh');
            
        }
        
    }
    
    public function mc_pe_anterior($idpe) {
        ...
        $data['modulos'] = $this->login_model->cargar_modulos2($a);
        $data['mapas_curriculares'] = $this->mapa_curricular_model->consultar_mapas_curriculares($idpe);
        $data['optativas']=$this->calificacion_model->consultar_optativas($idpe);
        $data['periodos'] = $this->periodo_model->lista_de_periodo();
        $data['noperiodos'] = $this->noperiodo_model->consultar_noperiodo();
         ...
         * donde nos encontramos
         */
        $data['breadcrumbs']['links'] = array(array('AIDE', '#'), array('Inicio', base_url() . 'app/inicio'), array('Planes de estudio', base_url() .'analista_servicios/gestion_planes_estudios/pe_anterior'), 'Mapa curricular');
         ...
         * Referencia al fragmento del contenido de esta sección
         */
        $data['app_fragment'] = $this->load->view('app/private/fragments/modules/Modulo2/analista_servicios/gestion_mapa_curricular_anterior_view', $data, TRUE);
    ...
    public function crear_mapa_curricular_anterior() {
    ...
    public function crear_optativas() {
        
            $idpe = $this->input->post('idpe');
            $array = json_decode($_POST['array']);
            
            $cont_optativas=0;
            foreach ($array as $idpermiso) {
                $arr_optativa = array();
                $arr_optativa['nuoptativa'] = $idpermiso[0];
                $arr_optativa['nomoptativa'] = $idpermiso[1];
                $arr_optativa['idpe'] = $idpe;

                $this->materias_model->crear_optativa($arr_optativa);
                $cont_optativas++;
            }
            echo json_encode(array("response_code" => 200, "optativas" => $cont_optativas));
    }
