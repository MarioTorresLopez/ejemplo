...
class gestion_grupos extends CI_Controller {
    ...
    public function index() {
        ...
        $data['acuerdos_institucion'] = $this->acuerdo_model->consultar_acuerdos_institucion($this->session->userdata('idusu'));
         ...
         */
        $data['app_fragment'] = $this->load->view('app/private/fragments/modules/Modulo2/usuario/acuerdos_usuario_view', $data, TRUE);
    ...
    public function grupo_acuerdo($idacuerdo) {
         ...
         * donde nos encontramos
         */
        $data['breadcrumbs']['links'] = array(array('AIDE', '#'), array('Inicio', base_url().'app/inicio'), array('Gestión de grupos', base_url().'usuario/gestion_grupos'), 'Agregar grupos');
         ...
         * Referencia al fragmento del contenido de esta sección
         */
        $data['app_fragment'] = $this->load->view('app/private/fragments/modules/Modulo2/usuario/gestion_grupo_usuario_view', $data, TRUE);
    ...
    public function crear_grupo() {
        ...
        if ($this->form_validation->run()) {
            ...
            $arr_nue_grupo['idacuerdo'] = $this->input->post('id_acuerdo');
            
            $idacuerdo = $this->input->post('id_acuerdo');
            $this->grupos_acuerdo_model->crear_grupos_acuerdo($arr_nue_grupo);
            
            redirect(base_url() . 'usuario/gestion_grupos/grupo_acuerdo/'. $idacuerdo, 'refresh');
    ...
    public function validar_grupo_acuerdo() {
        
        $idacuerdo = $this->input->post('id_acuerdo');

        $gruposA = $this->acuerdo_model->numero_grupos_acuerdo($idacuerdo);
        $gruposR = $this->grupos_acuerdo_model->cantidad_grupos_acuerdo($idacuerdo);
        $alumnosA = $this->acuerdo_model->numero_alumnos_acuerdo($idacuerdo);
        $alumnosR = $this->grupos_acuerdo_model->cantidad_alumnos_acuerdo($idacuerdo);
        
        $grupoautorizados = $gruposA->gruposautorizado;
        $gruposacuerdo = $gruposR->cangrupos;
        $alumnosautorizados = $alumnosA->alumnosautorizados;
        $alumnosacuerdo = $alumnosR->canalumnos;
               
        if ($grupoautorizados === $gruposacuerdo) {
            //Se hizo uso de todos los grupos
            echo '0';
        } else if ($alumnosautorizados === $alumnosacuerdo) {
            //Se hizo uso de todos los grupos
            echo '1';
        } else {
            //Aún hay grupos disponibles
            echo '2';
        }
